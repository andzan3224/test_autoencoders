...
data = open('data/RNN-train-data', 'r').read()  # Use this source file as input for RNN
...
# Hyper-parameters
hidden_size   = 200  # hidden layer's size
seq_length    = 100   # number of steps to unroll
learning_rate = 1e-4
...
while True:
    ...
    if n % 500 == 0:
        ...
        print("sample_seq is: {}".format(data[start_ix:start_ix + seq_length]))
        ixes          = []
        ...
        txt = ''.join(ix_to_char[ixl] for ixl in ixes)
        print('----\n %s \n----\n' % (txt,))
